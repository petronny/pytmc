...
class TmcClient(WebSocket, Event):
    ...
    async def on_open(self):
        ...
        await self.write_binary(message)

        await self.fire('on_open')

    async def write_binary(self, message):
        await self.write_message(message, True)
    ...
    async def on_message(self, data):
        try:
            ...
            await self.fire('parse_message_error')
            raise

        await self.fire('received_message')
    ...
    def _start_query_loop(self, **kwargs):
        ...
        def _query_message_loop(_self, group_name, _token):
            async def _():
                ...
                await _self.write_binary(query_message(token=_token))
